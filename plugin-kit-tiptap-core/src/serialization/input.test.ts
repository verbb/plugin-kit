...
describe('variableSerialization', () => {
    ...
    it('uses Unknown field instead of title-casing unresolved field UIDs', () => {
        const token = '{field:7aff44ed-bca7-4193-910b-6adf0d22fe57}';

        expect(resolveVariableTagLabel(token, null)).toBe('Unknown field');

        const attrs = resolveVariableTagByValue(token, TOP_LEVEL, ALL_VARIABLES);
        expect(attrs?.label).toBe('Unknown field');
        expect(attrs?.unresolved).toBe(true);
    });

    it('marks unresolved field tokens with sub-handles as unknown', () => {
        const token = '{field:261c777b-28ff-4025-a7b5-a308ba577c1a:repeaterText;scope=all}';
        const attrs = resolveVariableTagByValue(token, [], []);

        expect(attrs?.label).toBe('Unknown field');
        expect(attrs?.unresolved).toBe(true);
    });

    it('does not mark resolved field tokens as unresolved', () => {
        const attrs = resolveVariableTagByValue('{field:total}', TOP_LEVEL, ALL_VARIABLES);

        expect(attrs?.label).toBe('Total');
        expect(attrs?.unresolved).toBeFalsy();
    });
