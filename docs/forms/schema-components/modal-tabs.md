...
`ModalTabs` is a **schema component** (`$cmp: 'ModalTabs'`) that wraps stock kit `Tabs` / `Tab` / `TabPanel` with `variant="modal"` (`src/forms/components/ModalTabs.tsx`) and connects them to **schema form errors**.
