...
Product plugins often batch several product-owned `$field`s in one bootstrap helper (Formie’s `registerFormieOwnedSchemaFields` for `handle`, `list`, `richText`, `variablePicker`, `elementSelect`, etc.). Those keys are intentional omissions from Plugin Kit builtins.
