...
// → <Icon icon="plus"> / icon="gear" / icon="ellipsis" / icon="arrows-rotate"
```
...
Every built-in icon from `@verbb/plugin-kit-icons` — the canonical name list. Click a tile to copy its `<Icon>` tag. Generated from the package, so new glyphs appear here automatically. Prefer `arrows-rotate` for reload controls over `arrow-rotate-left` / `arrow-rotate-right`.
...
`<Icon>` is a square `1em` host with `vertical-align: -0.125em` so inline icons sit with surrounding text. Scale with `font-size`; no extra sizing CSS is required.
...
<ComponentPreview src="./examples/icon-alignment.preview.vue.ts" />
