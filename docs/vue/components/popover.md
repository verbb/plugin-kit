...
<ComponentPreview src="./examples/popover-basic.preview.vue.ts" />

## Placement

Position the popover on the side that best matches the surrounding control.

<ComponentPreview src="./examples/popover-placement.preview.vue.ts" />

## Form Content

Use compact form controls when the interaction should stay close to the trigger.

<ComponentPreview src="./examples/popover-form.preview.vue.ts" />

## With Arrow

`withArrow` draws a floating-ui arrow aimed at the trigger. Works with any `placement`.

<ComponentPreview src="./examples/popover-with-arrow.preview.vue.ts" />
...
| `flush` | Drop default panel padding/fixed width so slotted content (command lists, etc.) can control its own inset. Consumer outer `p-0` alone cannot reach `.panel`.<br><small><strong>Type</strong> <code>boolean</code></small><br><small><strong>Default</strong> <code>false</code></small> |
...
| `withArrow` | Show a floating-ui arrow on the panel (off by default). Named like other optional controls (`with-clear`, `with-caret`) — tooltip always paints its own arrow and has no toggle.<br><small><strong>Type</strong> <code>boolean</code></small><br><small><strong>Default</strong> <code>false</code></small> |
