...
| `withArrow` | Show a floating-ui arrow on the panel (off by default). Named like other optional chrome (`with-clear`, `with-caret`) — tooltip always paints its own arrow and has no toggle.<br><small><strong>Type</strong> <code>boolean</code></small><br><small><strong>Default</strong> <code>false</code></small> |
