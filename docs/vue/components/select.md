...
| `@pk-change` | Emitted when the selection changes.<br><small><strong>Type</strong> <code>{ value: string, values: string[] }</code></small> |
...
| Name | Description |
| --- | --- |
| `@pk-option-highlight` | Emitted when the option becomes the highlighted item. |
| `@pk-option-select` | Emitted when the option is activated (click / Enter). |
