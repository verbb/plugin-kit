...
<ComponentPreview src="./examples/radio-group-basic.preview.web.ts" />
...
<ComponentPreview src="./examples/radio-group-supporting-descriptions.preview.web.ts" />

## Disabled Options

Disable individual options when the broader choice is valid but one route is unavailable in the current context.

<ComponentPreview src="./examples/radio-group-disabled-options.preview.web.ts" />

## Layout and Error

Use horizontal layout for compact choices, and apply invalid state to the group when validation fails.

<ComponentPreview src="./examples/radio-group-layout-error.preview.web.ts" />
...
| `defaultValue` `value` | Default value — reflected as the `value` attribute.<br><small><strong>Type</strong> <code>string</code></small> |
