...
<ComponentPreview src="./examples/checkbox-basic.preview.web.ts" />

## Checked

Use checked state for persisted preferences so the current selection reads clearly at a glance.

<ComponentPreview src="./examples/checkbox-checked.preview.web.ts" />
...
<ComponentPreview src="./examples/checkbox-disabled.preview.web.ts" />
...
| `withHint` `with-hint` | Only required for SSR when slotting a hint.<br><small><strong>Type</strong> <code>boolean</code></small><br><small><strong>Default</strong> <code>false</code></small> |
