...
import { a as o, c as r, f as A, i as e, l as n, p as b, s as e$1, u as customElement } from "../../chunks/lit-DpLik9Rf.js";
import { c as __decorate, i as PkFormAssociatedElement } from "../../chunks/pk-base-CyzwylQ7.js";
import { Y as xmark } from "../../chunks/svg-_Mtb7CHx.js";
...
import { a as waitForPopupContentExitAnimation, i as syncFilteredOptions, n as matchesOptionFilter, r as AsyncOptionFetcher, t as pkComboboxStyles } from "../../chunks/pk-combobox.styles-DKUYtdek.js";
...
var PkAutocomplete = class PkAutocomplete extends PkFormAssociatedElement {
	...
	shouldShowAsyncEmpty() {
		...
		if (!this.value.trim() || this.asyncLoading || this.asyncError) return false;
		return this.getEnabledVisibleOptions().length === 0;
	}
	applyOptionState() {
	...
	openPanel() {
		...
		if (this.usesAsyncSearch) {
			this.asyncError = null;
			this.asyncLoading = false;
			this.scheduleAsyncFetch(this.value.trim());
		}
		this.style.setProperty("--pk-combobox-anchor-width", `${anchor.getBoundingClientRect().width}px`);
	...
	renderHostDecorationSlot(name) {
		...
		return b`<slot name=${name} part=${name} class="host-slot host-slot--${name}"></slot>`;
	}
	render() {
		const visibleOptions = this.getEnabledVisibleOptions();
		const showEmpty = this.open && (this.usesAsyncSearch ? this.shouldShowAsyncEmpty() : visibleOptions.length === 0);
		...
		return b`
            ...
            >
                ...
                <input
                    part="input"
                    class="combobox-input control-input"
                    type="text"
                    role="combobox"
                    id=${this.inputId}
                    .value=${this.value}
