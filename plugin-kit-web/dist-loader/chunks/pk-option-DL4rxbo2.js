...
import { n as renderIconHtml } from "./render-B_9MG170.js";
...
var PkOption = class PkOption extends PkElement {
	...
	*/
	...
	renderLabel() {
		...
		if (!query || this.hasRichLabelContent()) return b`
		...
		return b`
            <span part="label" class="label">
                ${splitHighlightParts(this.getLabel(), query).map((part) => part.match ? b`<mark class="match">${part.text}</mark>` : b`<span>${part.text}</span>`)}
            </span>
        `;
	}
	render() {
		return b`
            ...
            >
                ...
                ${this.renderLabel()}
                <span part="check" class="check" aria-hidden="true">${o(CHECK_ICON)}</span>
            </button>
        `;
	}
};
__decorate([n()], PkOption.prototype, "value", void 0);
__decorate([n()], PkOption.prototype, "label", void 0);
__decorate([n({
	type: Boolean,
	reflect: true
...
__decorate([n()], PkOption.prototype, "optionId", void 0);
__decorate([n({ attribute: false })], PkOption.prototype, "matchQuery", void 0);
PkOption = __decorate([t("pk-option")], PkOption);
//#endregion
