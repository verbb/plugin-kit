...
export class PkPopup extends PkElement {
    ...
    private settlingInitialPosition = false;
    ...
    reposition(): void {
        if (this.settlingInitialPosition) {
            return;
        }

        void this.repositionAsync();
    }
     ...
     */
    private async repositionAsync(emitEvent = true): Promise<boolean> {
        ...
        if (this.anchorElement) {
            // Use viewport rects for both sides — computePosition x/y can be offset-parent
            // relative, and mixing that with getBoundingClientRect shifts transform-origin
            // sideways (menu appears to slide in from the right).
     ...
     */
    private async settleInitialPosition(): Promise<void> {
        ...
        // Two frames: display:none→block + slotted panel unhide need a layout pass.
        await this.frames(2);

        if (!this.active || generation !== this.settleGeneration) {
            return;
        }

        // Quiet passes — consumers waiting on pk-reposition should see the settled place.
        await this.repositionAsync(false);

        // Confirm after the first coords are applied (size middleware / content height).
    ...
    stop(): Promise<void> {
        return new Promise((resolve) => {
            ...
            // Always clear — even when autoUpdate never started — so the next open
            // cannot short-circuit waitForPopupReposition on a stale placement.
    ...
    private start(): void {
        ...
        // Stay invisible until settleInitialPosition confirms flip with real content size.
        ...
        if (this.anchorTracking) {
            this.cleanup = autoUpdate(this.anchorElement, this.popupElement, () => {
                // Ignore the sync autoUpdate kick during settle — it races layout.
                if (this.settlingInitialPosition) {
                    return;
                }
        ...
        void this.settleInitialPosition();
    }
    ...
    override render() {
        ...
        return html`
            <slot name="anchor" @slotchange=${() => { void this.handleAnchorChange(); }}></slot>
