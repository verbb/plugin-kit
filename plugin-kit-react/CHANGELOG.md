...
### Fixed
- Fixed the React setup documentation to load ES modules in Craft, resolve published asset URLs, and provide complete starter examples.
...
- `Autocomplete` facade over `<pk-autocomplete>` — freeform text with optional
  suggestions (`onChange` value sugar, `isInvalid` alias).
- `AutocompleteInput` convenience wrapper (`options` / `fetchOptions`) and
  SchemaForm `$field: "autocomplete"` (`AutocompleteField`) for freeform path /
  alias settings with Field `warning` support.
...
### Removed
- Removed the unused `@verbb/plugin-kit-react/fault` surface (`AppFaultProvider`, `AppFaultBoundary`, `FaultFallback`, watchdog / support-bundle helpers). Prefer `AppErrorBoundary` instead.
